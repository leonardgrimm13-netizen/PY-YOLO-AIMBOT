...
- `python start.py` führt zuerst `update.run_prelaunch_update()` aus.
- Der Updater lädt zuerst `update_manifest.json` aus **diesem** Repository und vergleicht Hash + Dateigröße.
- Es werden nur geänderte Dateien heruntergeladen (inkl. `model.pt` nur bei Änderung).
- Downloads laufen in temporäre Dateien mit Hash-Prüfung und werden danach atomar ersetzt.
- Wenn GitHub nicht erreichbar ist, wird der lokale Start nicht blockiert.
- Lokaler Status wird in `.update_state.json` gespeichert (z. B. letzter Check/Update, Manifest-Metadaten).
- Falls Dateien aktualisiert wurden, startet `start.py` die App einmal sauber neu, um den neuen Stand zu laden.
